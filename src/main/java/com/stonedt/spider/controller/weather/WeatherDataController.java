package com.stonedt.spider.controller.weather;
...
//	private final String es_url = UtilConfig.getURL2().substring(0,UtilConfig.getURL2().length()-1);
	...
	private final String weatherlist_api = "/weather/weatherlist";
	
    private final String[] air_qualitys = new String[] {
	 ...
	 * 	质量筛选
	 */
	@RequestMapping(value = "/airQualitys", method = RequestMethod.POST, produces = "application/json;charset=utf-8")
	@ResponseBody
	public String airQualitys() {
		return JSON.toJSONString(air_qualitys);
	}

	/**
	 * 	列表
	 */
	@RequestMapping(value = "/weatherList", method = RequestMethod.POST, produces = "application/json;charset=utf-8")
	@ResponseBody
	public String weatherList(String pageNum, String airQuality, String time, String searchkeyword) {
		String times = "", timee = "";
		...
		map.put("air_quality", airQuality);
		map.put("size", "25");
		map.put("times", times);
		map.put("timee", timee);
		...
		try {
			response = esArticle(map, weatherlist_api);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ModelAndView mainstreamMediaList(ModelAndView mv) {
		...
		mv.setViewName("weatherData/weather-data");
		return mv;
	}
