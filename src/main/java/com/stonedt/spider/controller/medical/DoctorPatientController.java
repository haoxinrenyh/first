...
//	private final String es_url = UtilConfig.getURL2().substring(0,UtilConfig.getURL2().length()-1);
	...
	private final String doctorpatientbyclassify_api = "/doctorpatient/doctorpatientbyclassify";
	private final String doctorpatientlist_api = "/doctorpatient/doctorpatientlist";
	
	/**
	 * 	筛选
	 */
	@RequestMapping(value = "/doctorpatientbyclassify", method = RequestMethod.POST, produces = "application/json;charset=utf-8")
	@ResponseBody
	public String decorateLevels() {
		String url = es_url + doctorpatientbyclassify_api;
	 ...
	 * 	列表
	 */
	@RequestMapping(value = "/doctorpatientlist", method = RequestMethod.POST, produces = "application/json;charset=utf-8")
	@ResponseBody
	public String thesisnlist(String pageNum, String matchingmode, String time, String searchkeyword,
		...
		try {
			response = esArticle(map, doctorpatientlist_api);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ModelAndView mainstreamMediaList(ModelAndView mv) {
		mv.setViewName("medicalHealth/doctor-patient");
		return mv;
	}
