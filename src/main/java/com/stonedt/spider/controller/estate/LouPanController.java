...
//	private final String es_url = "http://s1.stonedt.com:6303";
	...
	private final String buildinglist_api = "/building/buildinglist";
	
	
	@Value("${URL2}")
    private  String es_url;
	
    private final String[] building_types = new String[] {
		"高层", "别墅|联体别墅", "小高层|高层", "别墅", "多层", "小高层", "多层|联体别墅", "联体别墅", "多层|小高层"
	};
    
	/**
	 * 	建筑类型筛选
	 */
	@RequestMapping(value = "/buildingTypes", method = RequestMethod.POST, produces = "application/json;charset=utf-8")
	@ResponseBody
	public String decorateLevels() {
		return JSON.toJSONString(building_types);
	}

	/**
	 * 	列表
	 */
	@RequestMapping(value = "/buildinglist", method = RequestMethod.POST, produces = "application/json;charset=utf-8")
	@ResponseBody
	public String buildinglist(String pageNum, String buildingType, String time, String searchkeyword) {
		String times = "", timee = "";
		...
		try {
			response = esArticle(map, buildinglist_api);
		} catch (Exception e) {
			e.printStackTrace();
