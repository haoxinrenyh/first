 ...
 */
@Controller
@RequestMapping("/thesisn")
public class ThesisnController {
...
//	private final String es_url = "http://s1.stonedt.com:6303";
	...
	private final String thesisnlist_api = "/thesisn/thesisnlist";
	private final String getthesisndetail_api = "/thesisn/getthesisndetail";
	
	/**
	 * 	列表
	 */
	@RequestMapping(value = "/thesisnlist", method = RequestMethod.POST, produces = "application/json;charset=utf-8")
	@ResponseBody
	public String thesisnlist(String pageNum, String matchingmode, String time, String searchkeyword) {
		...
		try {
			response = esArticle(map, thesisnlist_api);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String detail(String article_public_id) {
		String url = es_url + getthesisndetail_api;
		String params = "record_id=" + article_public_id;
		...
		if (StringUtils.isNotBlank(response)) {
			JSONObject parseObject = JSON.parseObject(response);
			result = JSONObject.parseObject(parseObject.toJSONString(), new TypeReference<Map<String, String>>(){});
		}
		return JSON.toJSONString(result);
	}
	...
	@RequestMapping(value = "/thesisn-page", method = RequestMethod.GET)
	public ModelAndView mainstreamMediaList(ModelAndView mv) {
		mv.setViewName("professorTalents/thesisn");
		return mv;
	}
	
	@RequestMapping(value = "/thesisn-detail", method = RequestMethod.GET)
	public ModelAndView detailPage(ModelAndView mv) {
		mv.setViewName("professorTalents/thesisn-detail");
		return mv;
	}
