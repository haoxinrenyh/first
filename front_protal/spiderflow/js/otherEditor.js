...
function Save() {
  validXML(function () {
    ...
    if (!$("#addTemplateModal").hasClass("show")) {
      $("#addTemplateModal").modal("show");
      return;
    }

    if (!$("#template_name").val()) {
      $(".message-error .message_content").html("请输入模板名称");
    ...
    if (ajaxSaveStatus) {
      ...
      $.ajax({
        url: getRealPath() + "/template/save",
        type: "post",
        contentType: "application/json",
        data: JSON.stringify({
          id: templateId,
          template_name: $("#template_name").val(),
          type_id: JSON.parse($("#data_type_id").val()),
          xml: editor.getXML(),
          status: 0,
        }),
