      ...
      $.ajax({
        ...
        success: function (res) {
          if (res.code == 200) {
            ...
            if (xml.temp_id) {
              template_id = xml.temp_id;
            }

            if (xml.seed_name) {
              $("#seed_name").val(xml.seed_name);
            }
            if (xml.cron) {
              $("#cron").val(xml.cron);
            }
            if (xml.seed_type) {
              $("#seed_type").val(xml.seed_type);
            }
            if (xml.seed_status != null) {
              $("#seed_status").val(xml.seed_status);
            }

            if (xml.data_type_id) {
              data_type_id = xml.data_type_id;
            }

            if (xml.spider_level) {
              $("#spider_level").val(xml.spider_level);
            }
            if (xml.sole_sign) {
              $("#sole_sign").val(xml.sole_sign);
            }

            if (xml.key_sources_flag) {
              $("#key_sources_flag").val(xml.key_sources_flag);
            }

            if (xml.type != null) {
              $("#gather_type").val(xml.type);
            }

            if (xml.spider_type) {
              $("#spider_type").val(xml.spider_type);
            }

            // if (xml.customlable) {
            //   arrdata = xml.customlable.split(" ");
...
function bindToolbarClickAction(editor) {
  ...
  $(".toolbar-container")
    ...
    .on("click", ".btn-return", function () {
      ...
      if (currentPath == "/page/collectTask/task.html") {
    ...
    .on("click", ".btn-dock-right", function () {
      ...
      resize.onmousedown = function (e) {
        ...
        document.onmousemove = function (e) {
          ...
          $(".properties-container").width(box.clientWidth - moveLen - 260);
          var fatherElement = $(".main-container").width();
          $(".editor-container,.xml-container").width(
            fatherElement - (box.clientWidth - moveLen - 260)
          );
          /*  $(".xml-container").width(
    ...
    .on("click", ".btn-dock-bottom", function () {
      ...
      resize.onmousedown = function (e) {
        ...
        document.onmousemove = function (e) {
          ...
          if (moveLen > maxT - 150) moveLen = maxT - 150;
          resize.style.top = moveLen - 140 + "px";
          resizeSlideBar();
          ...
          $(".editor-container,.xml-container").height(
            fatherElement - (box.clientHeight - moveLen - 80)
          );
          $(".properties-container").height(box.clientHeight - moveLen - 80);
        };
        document.onmouseup = function (evt) {
          document.onmousemove = null;
          document.onmouseup = null;
...
function Save() {
  validXML(function () {
    ...
    if (!$("#spider_website").val()) {
      $(".message-error .message_content").html("请选择采集站点");
